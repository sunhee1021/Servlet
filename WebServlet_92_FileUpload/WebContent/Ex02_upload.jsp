...
<%
	...
	MultipartRequest multi = new MultipartRequest(
			request, //javax.servlet.http.HttpServletRequest request 주소값을 의미
			uploadpath, //실 저장 경로 
			size,		//최대 10M
			"UTF-8",    //encoding 
			new DefaultFileRenamePolicy()  //파일정책에 대해서 Rename을 줬어, 덮어쓰느냐 새로운 파일을만드느냐
										   //-> 파일 중복이 걸리면 (upload  >  a.txt 업로드 > a.txt를 또 업로드)
										   //-> 같은 파일이니까 충돌나겠지
										   //-> 자동으로 물어보지 않고 이름을 변경하게 할거야 ( ex) a_1.txt) 
										   //a 랑 txt 를 split으로 분리해서 a뒤에 숫자를 붙인다음에 txt랑 다시 붙여!
			);
			
	//파일 업로드 완료(6종)
	//여기까지 코드가 서버에 upload 폴더!!!에 파일이 저장
	//db서버는 파일의 경로만 갖지, 파일자체를 저장하지 않아요, 미친짓이죠
			
	//자료실
	//2가지
	//1. 서버에 파일 업로드
	//2. 작성자, 제목, 내용, 파일명, 파일사이즈 같은것들을 DB에 insert 하는 작업이 필요
	...
	//getFileNames
	//form 요소 중 input 태그에서 file 속성으로 지정된 태그의 name 속성의 값
	//Enumeration 객체 타입으로 반환한다
	
	//<input type="file" name="filename1">
	//<input type="file" name="filename2">
	//while(filenames.hasMoreElements()){  //element를 가지고 있다면 while문을 돌아
	//	System.out.println(filenames.nextElement()); 
		//input 태그의 name 값을 뽑아낸다(콘솔에)
		//순서는 없다, key값으로 뽑아내는 개념이라		}
	...
	//getFilesystemName()
	//사용자가 지정한 파일이름이 아니고
	//사용자가 선택한 파일이 실제 서버상의 폴더에 저장되었을때의 파일명을 반환
	//파일명이 중복되면 시스템에서 자동으로 바꿔주는데 (DefaultFileRenamePolicy에 의해)
	//그 이름을 말함
	...
	//기존 폴더안에 a.jpg 가 있다
	//이름이 같은 새로운 파일을 업로드 (a.jpg)
	//그러면 여기서 orifilename2 (origin) 은 a.jpg
	//정책(policy)에 의해서 만들어지는 파일이 > filename2 > a1.jpg
